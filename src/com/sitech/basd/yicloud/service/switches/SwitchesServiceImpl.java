package com.sitech.basd.yicloud.service.switches;

import java.util.List;

import com.sitech.basd.yicloud.dao.switches.SwitchesDao;
import com.sitech.basd.yicloud.domain.switches.SwitchObj;

public class SwitchesServiceImpl implements SwitchesService {
	/**
	 * 
	 * @Title: queryForListByObj
	 * @Description:查询列表（包括路由器和交换机）
	 * @param
	 * @return List
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Aug 28, 2012 11:36:50 AM
	 */
	public List queryForListByObj(SwitchObj obj) {
		return switchesDao.queryForListByObj(obj);
	}

	/**
	 * 
	 * @Title: insertByObj
	 * @Description: 添加一条记录
	 * @param
	 * @return int
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Aug 28, 2012 11:20:10 AM
	 */
	public int insertByObj(SwitchObj obj) {
		return switchesDao.insertByObj(obj);
	}

	/**
	 * 
	 * @Title: updateByObj
	 * @Description: 修改一条记录
	 * @param
	 * @return int
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Aug 28, 2012 11:20:10 AM
	 */
	public int updateByObj(SwitchObj obj) {
		return switchesDao.updateByObj(obj);
	}

	/**
	 * 
	 * @Title: deleteByObj
	 * @Description: 删除一条记录
	 * @param
	 * @return int
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Aug 28, 2012 11:20:10 AM
	 */
	public int deleteByObj(SwitchObj obj) {
		return switchesDao.deleteByObj(obj);
	}

	/**
	 * 
	 * @Title: queryByObj
	 * @Description: 查询一条记录
	 * @param
	 * @return SwitchObj
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Aug 28, 2012 11:22:30 AM
	 */
	public SwitchObj queryByObj(SwitchObj obj) {
		return switchesDao.queryByObj(obj);
	}

	/**
	 * 
	 * @Title: getIdSequence
	 * @Description: 查询路由器（交换机）的ID
	 * @param
	 * @return String
	 * @throws
	 * @author taoxue
	 * @version 1.0
	 * @createtime Jul 18, 2012 9:44:10 AM
	 */
	public String getIdSequence() {
		return switchesDao.getIdSequence();
	}

	private SwitchesDao switchesDao;

	public void setSwitchesDao(SwitchesDao switchesDao) {
		this.switchesDao = switchesDao;
	}

}
