...
@Repository("reportFormVmDao")
public class ReportFormVmDaoImpl extends BaseDao implements ReportFormVmDao {
	//先确定表名，后续通过表名进行查询
		...
		try {
			if (obj.getPagination() != null) {
				...
				obj.getPagination().setTotalCount(
						((Integer) getSqlMap().queryForObject("ReportForm.queryAllVirtualCount", obj))
								.intValue());
			}
			if(obj.getIsForAdd()==1){
				lst = getSqlMap().queryForList(
						"ReportForm.queryVMReportFormDataTwo", obj);
			}else{
				lst = getSqlMap().queryForList(
						"ReportForm.queryVMReportFormData", obj);
			}
		} catch (Exception sqlexception) {
			sqlexception.printStackTrace();
			LogHelper.error("VmReportForm.queryVMReportFormData:"
		...
		try {
			if("top".equals(obj.getTop_line())){
				list = getSqlMap().queryForList(
					"ReportForm.queryVMIndexNameLable_cpu", obj);
			}else{
				list = getSqlMap().queryForList(
						"ReportForm.queryVMIndexNameLable__zx",obj);
			}
		} catch (SQLException e) {
			LogHelper.error("ReportForm.queryVMIndexNameLable_cpu:"
	...
	public List<Data> queryDataByCurrentDayForChart_mem(VmReportForm obj) {
		...
		try {
			...
			list = getSqlMap().queryForList(
					"ReportForm.queryVMIndexNameLable_mem", obj);
		} catch (SQLException e) {
			LogHelper.error("ReportForm.queryVMIndexNameLable_mem:"
					+ e.getMessage() + getClass().getName());
		}
		return list;
	}
	...
	public List<VmReportForm> queryDataByCurrentDayForChartList(VmReportForm obj) {
		...
		try {
			lst = getSqlMap().queryForList(
					"ReportForm.queryVMReportFormDataForChartList", obj);
		} catch (Exception sqlexception) {
			sqlexception.printStackTrace();
			LogHelper.error("VmReportForm.queryVMReportFormDataForChartList:"
					+ sqlexception.getMessage() + getClass().getName());
		}
		return lst;
	}
	...
	public List<Data> queryDataByCurrentDayForChart_time(VmReportForm obj) {
		...
		try {
			list = getSqlMap().queryForList(
					"ReportForm.queryVMIndexNameLable_time", obj);
		} catch (SQLException e) {
			LogHelper.error("ReportForm.queryVMIndexNameLable_time:"
					+ e.getMessage() + getClass().getName());
		}
		return list;
	}
	...
	public List<VmReportForm> queryDataFromAgentData(VmReportForm obj){
		...
		try {			
			list = getSqlMap().queryForList(
					"ReportForm.queryVMReportFormAgentData", obj);
		} catch (SQLException e) {
			LogHelper.error("ReportForm.queryVMReportFormAgentData:"
					+ e.getMessage() + getClass().getName());
		}
		return list;
	}
		...
		public List<VmReportForm> queryVmhostList(VmReportForm obj) {
			...
			try {
				if (obj.getPagination() != null) {
					...
					obj.setPAGESIZE(obj.getPagination().getPageSize());
					obj.getPagination().setTotalCount(
							((Integer) getSqlMap().queryForObject("ReportForm.queryVmhostListForCount", obj))
									.intValue());
				}
				list = getSqlMap().queryForList(
						"ReportForm.queryVmhostList", obj);
			} catch (SQLException e) {
				LogHelper.error("ReportForm.queryVmhostList:"
						+ e.getMessage() + getClass().getName());
			}
			return list;
		}
