	...
	<script type="text/javascript">
		$(function(){
	 	...
	 	function addRequest() {
	 		$.dialog({
				...
				min : false,
				lock : true,
				content : 'url:osGroup_addGroup.do?theForm.os_host_id=' + theForm.os_host_id.value
			});
	 	}
		...
		function modRequest() { 
			...
			$.dialog({
				...
				min : false,
				lock : true,
				content : 'url:osGroup_updateGroup.do?theForm.id=' + theForm.id.value
			});
		}
		...
		function delRequest() {
			...
			if(confirm("确定要删除?")){
				$.ajax({
				type : 'post',
				url : 'osGroup_deleteGroup.do?theForm.id=' + theForm.id.value,
		...
		function _save_button_click_event(argForm){
			$.ajax({
				type : 'post',
				url : 'osGroup_saveGroup.do?' + argForm,
				success : function(msg) {
					if (msg == -1) {
						alert("保存失败");
					} else {
		...
		function _update_button_click_event(argForm){
			$.ajax({
				type : 'post',
				url : 'osGroup_modifyGroup.do?' + argForm,
				success : function(msg) {
					if (msg == -1) {
						alert("保存失败");
					} else {
...
<body>
<div class="mainbody">
	<s:form action="osGroup_queryGroup.do" method="post" cssStyle="theForm" id="theForm">
	...
	<div class="pd-20 bgcolor-1">
		...
		<div class="bord-1 pd-10">
			...
			<table id="theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
				<thead>
					<tr>
						<th width="40%" onclick="sort(theTable,0,'string')">组ID</th>   
						<th width="30%" onclick="sort(theTable,1,'string')">组标识</th>
						<th width="30%" onclick="sort(theTable,2,'string')">描述</th>
					</tr>
				</thead>
				<tbody>
				<s:iterator value="theForm.otglist" id="theBean">
					<tr>
			...
			<table id="theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
				<thead>
					<tr>
						<th width="5%">编号</th>
						<th width="35%" onclick="sort(theTable,0,'string')">组ID</th>   
						<th width="30%" onclick="sort(theTable,1,'string')">组标识</th>
						<th width="30%" onclick="sort(theTable,2,'string')">描述</th>
					</tr>
				</thead>
				<tbody>
				<s:iterator value="theForm.oglist" id="theBean">
					<tr>
