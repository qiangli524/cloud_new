...
<script type="text/javascript">
	...
	function comp_config(templ_id,type){
		//alert("ID:"+templ_id+",TYPE:"+type);
		var url = "";
		if(type == '1'){//基本信息
		
		}else if(type == '2'){//文件系统
			url = 'osTemplComp_showOsTemplateFileSystem.do?theForm.template_id=' + templ_id;
		}else if(type == '3'){//组
			url = 'osTemplComp_showOsTemplateGroup.do?theForm.template_id=' + templ_id;
		}else if(type == '4'){//用户
			url = 'osTemplComp_showOsTemplateUser.do?theForm.template_id=' + templ_id;
		}else if(type == '5'){//附加组
			url = 'osTemplComp_showOsTemplateGroupUser.do?theForm.template_id=' + templ_id;
		}else if(type == '6'){//软件
			url = 'osTemplComp_showOsTemplateSoft.do?theForm.template_id=' + templ_id;
		}else if(type== '7'){//Part配置
			url = 'osTemplComp_showOsTemplatePart.do?theForm.template_id=' + templ_id;
		}else if(type== '8'){//卷组配置
			url = 'osTemplComp_showOsTemplateVolGroup.do?theForm.template_id=' + templ_id;
		}
		if(url == ''){
			alert("未使用该项配置");
			return false;
	...
	function returnFunction(){
		...
		$(_iframe).attr( "src", "osTemplate_showOsTemplate.do");
	}
...
<s:form action="osTemplate_showOsTemplate.do" method="post" id="theForm">
	<div class="content_wrap">
		<div class="left">
			...
			<div>
				<div>
					<input type="button"  value="返回" onclick="returnOsTemplate()"/>
				</div>
			</div>
			 -->
			<ul id="configComp" class="ztree">
				...
				<li><a href="javascript:void(0);return false;" onclick="javascript:comp_config('<s:property value="theForm.id"/>','7');">Part配置</a></li>
				<li><a href="javascript:void(0);return false;" onclick="javascript:comp_config('<s:property value="theForm.id"/>','8');">卷组配置</a></li>
				...
				<li><a href="javascript:void(0);return false;" onclick="javascript:comp_config('<s:property value="theForm.id"/>','6');">软件包配置</a></li>
		...
		<div class="right">
			<div id="iframe">
				...
				--%>
				<iframe id="right_iframe" src="osTemplComp_showOsTemplatePart.do?theForm.template_id=<s:property value="theForm.id"/>" name="xxxx"  width="100%" frameborder="0"></iframe>
			</div>
		</div>
	</div>
</s:form>
