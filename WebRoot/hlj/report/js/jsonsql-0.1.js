...
var jsonsql = {
	...
	query: function(sql,json,islike){
	...
	returnFilter: function(json,jsonsql_o){
		...
		var val = "";
		var key = "";
		if(jsonsql_o.where!='true'){			
			var ws = jsonsql_o.where.split("==");
			val = ws[1].replace(' ','').replace(new RegExp(/(')/g),'');
			key = ws[0].replace(' ','');
		}
		for(var jsonsql_i in jsonsql_scope){
			var obj = jsonsql_scope[jsonsql_i]
			var isreturn = false;
			if(jsonsql_o.islike){
				if((obj[key]!=undefined && obj[key].indexOf(val)>=0) || jsonsql_o.where=='true'){
					isreturn =true;
				}
			}else{				
