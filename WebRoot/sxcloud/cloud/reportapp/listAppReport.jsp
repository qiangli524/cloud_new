...
<script type="text/javascript">
	...
	function addRequest() {
		theForm.flag.value = 0;
 	    theForm.action = 'addAppMessage.do' 
		theForm.submit();
	}
	function modRequest() { 
 	    var couterNum = 0;
 	    ...
 	    theForm.action = 'modAppMessage.do' 
		theForm.submit();
 	}
 	function delRequest() {
 	var couterNum = 0;
 	    ...
 	    theForm.action = 'delAppMessage.do'  
		theForm.submit();
 	}
</script>
</head>
<body>
<s:form action="reapp_listAppReport.do" method="post" cssClass="theForm" id="theForm">
        ...
        <div class="query-form">
                <table width="100%" class="querytable" border="0">
                  <tr>
                    ...
                    <td>
                    	<s:textfield name="theForm.APP_ID" cssClass="txt" id="APP_ID"></s:textfield>
                    </td>
	...
	<div class="blue-wrap noborder">
		...
		<div class="table-ct">
			<table id="theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
			  <thead>
			  <tr>
					<th onclick="sort(theTable,0,'string')">应用编号</th>
					<th onclick="sort(theTable,1,'string')">应用名称</th>
					<th onclick="sort(theTable,2,'string')">进程名1</th>
					<th onclick="sort(theTable,3,'string')">进程名2</th>
					<th onclick="sort(theTable,4,'string')">协议类型</th>
					<th onclick="sort(theTable,5,'string')">所占内存</th>
					<th onclick="sort(theTable,6,'string')">状态</th> 
					<th onclick="sort(theTable,7,'string')">所属主机</th>
					<th onclick="sort(theTable,8,'string')">所属系统名称</th>
					<th onclick="sort(theTable,9,'string')">应用类型</th> 
					<th>查看报表</th>  
			  </tr>
			  </thead>
			  <tbody>
			  <s:iterator value="theForm.resultList" id="theBean">
			  	<tr>
			  		...
			  		<td>
			  			...
			  			<s:elseif test="#theBean.APP_TYPE==1">虚拟主机</s:elseif>
			  		</td>
			  		 <td>
						  	<a href="reapp_getAppCurrentReportView.do?app_id=<s:property value="#theBean.APP_ID"/>">
							查看报表
							</a>
					</td>
			  	</tr>
