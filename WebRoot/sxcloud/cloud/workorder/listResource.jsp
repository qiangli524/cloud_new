  ...
  <script type="text/javascript">
		...
		$(function(){
			$("[name='dealbyhand']").unbind().live("click",function(){
				...
				$.ajax({
					type:'post',
					url:'workorder/workorder_dealByHand.do?wrid='+wrid,
			...
			$("[name='deploy']").unbind().live("click",function(){
				...
				if (confirm("你确定该虚拟机已经挂载存储并能够交付使用了吗？")) {
					$.ajax({
						type:'post',
						url:'workorder/workorder_deployVM.do?wrid='+wrid,
						success:function(msg){
							if (msg == -1) {
								alert("很遗憾，该虚拟机交付失败！");
							} else {
			...
			$("#addForm").click(function(){
				...
				w.$.dialog({
					...
					lock: true,
					parent:api,
					content:'url:workorder/workorder_addResource.do?operFrom=addResource&oper=add&type='+type+'&uuid='+uuid
				});
			});
			...
			$("#editForm").click(function(){
				...
				if(status == 0 || status == 3){
					...
					w.$.dialog({
						...
						height:'500px',
						lock: true,
						content:'url:workorder/workorder_editResource.do?operFrom=editResource&wrid='+wrid+'&oper=edit&type='+type+'&uuid='+uuid
					});
				}else{
			...
			$("#deleteForm").click(function(){
				...
				if (confirm("你确定要进行删除操作吗？")) {
					$.ajax({
						type:'post',
						url:'workorder/workorder_deleteResource.do?wrid='+wrid,
						success:function(msg){
							if (msg == -1) {
								alert("删除失败");
							} else {
			...
			$("[name='resourceDetail']").unbind().live("click",function(){
				...
				w.$.dialog({
					...
					height: '500px',
					lock: true,
					content: 'url:workorder/workorder_resourceDetail.do?wrid='+wrid
				});
			});
		...
		function saveResource(theForm){
			$.ajax({
				type:'post',
				url:'workorder/workorder_saveResource.do',
				data:theForm,
				success:function(msg){
					if (msg == -1) {
						alert("保存失败");
		...
		/*add by qism 插入EntityUser表中*/
		function saveEntityUser(os){
			$.ajax({
				type:'post',
				url:'workorder/workorder_saveEntityUser.do?ios='+os,
				success:function(msg){
					if (msg == -1) {
						alert("没有使用该操作系统的主机!");
					}else if(msg == -2){
...
<body>
	<div class="mainbody">
	<s:form action="workorder/workorder_listResource.do" method="post" id="theForm" cssStyle="theForm">
	...
	<s:hidden name="state" id="state"></s:hidden>
	 	<div class="pd-20 bgcolor-1">
			...
			<div class="utt-2 mgt-20">
				...
				<a class="icon-occupy" href="javascript:void(0)"  onclick="viewResourceClose()">返回</a>
				<a class="icon-release" href="javascript:void(0)"  onclick="refresh()">刷新</a>
			</div>
				<table id="theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
					<thead>
						<tr>
							...
							<th onclick="sort(theTable,11,'string')">创建进度</th>
					...
					<tbody>
						<s:iterator value="resultList" id="theBean">
							<input type="hidden" id="ifList" name="ifList" value="1" />
			  				<tr>
			  					<td>
			  						<input type="checkbox" name="checkboxid" value="<s:property value='#theBean.ID'/>" status='<s:property value="#theBean.STATUS" />'/>
			  					</td>
			  					<%--  
			  					<td>
			  					...
			  					<td>
			  					<span class="js_ByEllipsis" title="<s:property value='#theBean.PROJECT_NAME'/>"><s:property value="#theBean.PROJECT_NAME"/></span>
<%--			  						<s:property value="#theBean.PROJECT_NAME"/>--%>
			  					</td>
				  				<td><s:property value="#theBean.CPU_NUM" />核
											</td>
				  				<td>
				  				...
				  				<td>
				  					<span class="js_ByEllipsis" title="<s:property value='#theBean.IPADDRESS'/>"><s:property value="#theBean.IPADDRESS"/></span>
<%--				  					<s:property value="#theBean.IPADDRESS" />--%>
								</td>
								<td>
									<span class="js_ByEllipsis" title="<s:property value='#theBean.TEMPLATENAME'/>"><s:property value="#theBean.TEMPLATENAME"/></span>
<%--									<s:property value="#theBean.TEMPLATENAME"/>--%>
								</td>
								<td>
									<s:if test="#theBean.TEMP_TYPE==1">
									VMWARE
									</s:if>
									<s:if test="#theBean.TEMP_TYPE==2">
									XEN
									</s:if>
								</td>
								<td>
											<s:if test="#theBean.proGress!=null">
								...
								<td><s:property value="#theBean.DEALTIME" default="---"/></td>
								<td><s:property value="#theBean.DEAL_COUNT" /></td>
								<td wrid='<s:property value="#theBean.ID"/>' vmname = '<s:property value="#theBean.VM_NAME"/>'>
									<s:if test="state==0">
										...
										<s:if test="#theBean.proGress!='100'">
											<input type="hidden" id="ifProcess" name="ifProcess" value="1" />
										</s:if>
									</s:if>
										&nbsp;&nbsp;&nbsp;
									<a href="javascript:;" name="resourceDetail">查看</a>
								</td>
			  				</tr>
		  				</s:iterator>
					</tbody>
...
<script type="text/javascript">
	if($("#ifList").val()=="1"&&$("#ifProcess").val()=="1"){
