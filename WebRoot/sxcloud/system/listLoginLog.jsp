...
<body onLoad="self.focus();document.theForm.ACCOUNT.focus()">
	<div class="mainbody">
		<s:form action="log_listLoginLogList" method="post" cssStyle="theForm"
			id="theForm">
			<s:hidden name="theForm.ID" id="ID" />
			<div class="pd-20 bgcolor-1">
				<h2 class="utt-1">登陆日志</h2>
				<div class="bord-1 pd-10">
					<div class="clearfix filtrate-area">
						...
						<div class="filtrate-field">
							...
							<s:textfield name="theForm.IP" id="IP" cssClass="inpt-1 vm"
								maxlength="30"></s:textfield>
						</div>
						<div class="filtrate-field">
							<label class="vm">终端名称 ：</label>
					...
					<table id="theTable" width="100%" class="blue-table sorttable"
						...
						<thead>
							<tr>
								...
								<th onclick="sort(theTable,1,'string')">登录IP地址</th>
								<th onclick="sort(theTable,2,'string')">登录终端名称</th>
								<th onclick="sort(theTable,3,'date')">登录时间</th>
								<th onclick="sort(theTable,4,'date')">退出时间</th>
								<th onclick="sort(theTable,5,'string')">结果</th>
						...
						<tbody>
							<s:iterator value="theForm.resultList" id="theBean">
								<tr>
									...
									<td><s:if
											test="#theBean.HOSTNAME==null ||#theBean.HOSTNAME==''">未获取到信息</s:if>
										<s:else>
											<s:text name="#theBean.HOSTNAME" />
										</s:else></td>
									<td><s:text name="#theBean.LOGINTIME" /></td>
									<td><s:if test="#theBean.LOGOUTTIME!=null">
											<s:text name="#theBean.LOGOUTTIME" />
										</s:if> <s:else>
												未成功获取信息
											</s:else></td>
									<td><s:if test="#theBean.RESULT==0">
												...
												成功
											</s:elseif> <s:elseif test="#theBean.RESULT==2">
											未知
											</s:elseif></td>
