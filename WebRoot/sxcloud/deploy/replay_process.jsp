...
<!--	})-->
   ...
   //    var checkboxids = document.getElementsByName("checkboxid");
    //   if(checkboxids!=null&&checkboxids.length>0){
     ...
     //  }
     	var videoid = 1;
     	$.getJSON("depcomm_reploy_process_time.do?videoid=" + videoid,{'time':new Date().toString()},function(data){
     		var result='';
     		for(j=0;j<data.length;j++){
     			if(data[j].FLAG==1){
     				data[j].CONTENT = '[root]# ' + data[j].CONTENT;
     			}
     			result +=data[j].CONTENT + "</br>";
			}
			$("#cmd").html(result);
     	});
    }
    ...
    --%>
    function timer1(ii,count){
      i++;
      if(ii<count){
        setTimeout("foreach()",1000);
      }
    }
    //点击'查询'按钮出发的js
    function searchRequest() {
       ...
       if(end_time==null||end_time==0){
         ...
         return;
   		...
   		theForm.action = 'reploy_processByObj.do';
		theForm.submit();
 	}
</script>
...
<!--				   <td width="10%">bainhao:</td>-->
				   ...
				   <td width="62%"><font color=white>------------记录------------</font></td>
