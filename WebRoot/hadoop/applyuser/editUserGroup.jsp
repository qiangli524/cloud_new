...
<script type="text/javascript">
	...
	function saveInfo(){ 
		...
		if (groupname.length == 0) {
			$("#namespan").append("<font color='red'>ERROR：请填写用户组名称</font>");
			return false;
		}
		...
		var flag = true;
		mask('正在检测用户组是否存在，请稍后','0.7','0px');
		$.ajax({
			type:'post',
			url:'hadoopUserGroup_checkIfExsit.do?hadoopUserGroup.name='+groupname+'&hadoopUserGroup.cluster_id='
					+clusterId+'&hadoopUserGroup.serviceArr='+serviceArr,
			async:false,
			dataType:'json',
			success:function(msg){
				...
				if (msg.ret == -1) {
					flag = false;
					alert("该服务下该用户组已经存在，不能重复添加，请取消该操作或者更改用户组名称");
				}
			}
		});
		if (flag) {
			w.saveUserGroup($("#theForm").serialize());
		}
	}
		...
		<div>
			<table width="100%" border="0" cellspacing="0" class="pop-table nosize">
					...
					<td>
						<s:if test="oper == 'add'">
							<s:select list="clusterList" headerKey="-1" headerValue="--请选择--" listKey="cluster_id" listValue="cluster_name"
								name="hadoopUserGroup.cluster_id" id="clusterId"></s:select>
							<span id="clusterspan"></span>
						</s:if>
						<s:else>
							<s:hidden name="hadoopUserGroup.cluster_id" id="editCluster"></s:hidden>
							<s:textfield name="hadoopUserGroup.cluster_name" readonly="true" disabled="true"></s:textfield>
						</s:else>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<s:textarea name="hadoopUserGroup.description" id="descr" rows="4" cols="40"></s:textarea>
					</td>
				</tr>
			</table>
		</div>
