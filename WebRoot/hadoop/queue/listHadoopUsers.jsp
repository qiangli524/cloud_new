...
<head>
	<script type="text/javascript">
	    ...
	    $(function(){
	    	...
	    	$("#assoUser").click(function(){
	    		var oper = $("#oper").val();
	    		if(oper=="add"){
	    			theForm.action='hadoopQueue_selectUsers.do?flag=show';
		    		theForm.submit();
	    		}else{
	    			theForm.action='hadoopQueue_managerUsers.do?flag=show';
		    		theForm.submit();
	    		}
	    	...
	    	$("#assoGroup").click(function(){
	    		var oper = $("#oper").val();
	    		if(oper=="add"){
	    			theForm.action='hadoopQueue_selectUsersGroup.do?flag=hide';
		    		theForm.submit();
	    		}else{
	    			theForm.action='hadoopQueue_managerUsersGroup.do?flag=hide';
		    		theForm.submit();
	    		}
	    ...
	    function selectUser(){
				...
				api.get("addqueue").saveUser(serviceId);
	    }
	    
	    function connect(){
	    	var queueId = $("#idstr").val();
	    	w.$.dialog({
	    		id:'connectUser',
				title:'关联用户',
				width:'800px',
				height:'500px',
				lock:true,
				content:'url:hadoopQueue_connectUser.do?idstr='+queueId+'&flag=show'
	    	});
	    }
	    ...
	    function enconnect(){
	    	...
	    	var url = 'hadoopQueue_deleteConnectUser.do?hadoopQueueRelationObj.entity_id='+serviceId;
			$.ajax({
	            type: "POST",
	            url: url,
	            async:false,
	...
	    	});
	    ...
	    function saveConnectUser(serviceId,queue_id){
	    	var url = 'hadoopQueue_saveConnectUser.do?hadoopQueueRelationObj.queue_id='+queue_id+'&hadoopQueueRelationObj.entity_id='+serviceId;
			$.ajax({
	            type: "POST",
	            url: url,
	            async:false,
			...
			<div class="box on">
				...
				<div class="blue-wrap noborder">
					<div class="table-head">
						 <ul class="btns" id="showOrHide">
							<li><input type="button" class="thickbox btn-style02" value="关联" onclick = "connect();return false;" /></li>
							<li><input type="button" class="thickbox btn-style02-75" value="取消关联" onclick = "enconnect();return false;" /></li>
						</ul>
