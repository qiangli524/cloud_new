...
<head>
	...
	//	});	
		...
		jConfirm(content,'确认信息', function(r) {
			...
			if(r==true){
				bar(ID,"正在创建虚拟机"+name_en);
				$.getJSON("vmw_deployVirtualMachine.do?ID="+ID+'&name_en='+encodeURI(encodeURI(name_en))+'&currentmemory='+currentmemory+'&UNIT='+UNIT+'&currentcpu='+currentcpu+'&TYPE='+TYPE +'&osId='+osId +'&dataName='+dataName+'&dataSize='+dataSize + '&osName=' + osName + '&NET=' + NET + '&IP=' + IP + '&netType=' + netType+'&PARENT_ID='+PARENT_ID,{'time':new Date().toString()},function(data){		
					var result = data.result;
					if (result == 1) {
						barEnd(ID,"创建虚拟机"+name_en+"成功");
						window.parent.refreshParentNode();
						//window.document.location.reload();
					} else {
						barEnd(ID,"创建虚拟机"+name_en+"失败");	
	...
	function getDSInfo() {
		...
		$.getJSON("vmw_getDSInfo.do?dsname="+dsname,{'time':new Date().toString()},function(data){	
		...
		<div>
			<table width="100%" border="0" cellspacing="0"
				...
				<tr>
					...
					<td>
						<s:select list="theForm.resultList" listKey="NAME" listValue="NAME" name="theForm.dataId" id="STORAGE" onchange="getDSInfo()"></s:select>
					</td>
				</tr>
				<tr>
					<td class="til" align="center">
