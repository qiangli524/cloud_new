...
<head>
	<script type="text/javascript">
		...
		var title = '<%=request.getAttribute("vmName")%>';
   ...
   /* Position the console */
				...
				// Start up VMRC
  				...
  				// Adjust CSS geometry
   				...
   				if (vmrc != null && vmrc.isReadyToStart != null) {
     				...
     				$("#hint").html("请先安装<a href='javascript:void(0)' onclick='vmrcDownload()'>vmware-plug-in</a>,安装以后<a href='javascript:void(0)' onclick='reload()'>刷新</a>页面");
   				}
			}
		});
		//xugang 20140701
		//下载vmware虚拟机远程控制台
		function vmrcDownload(){
		...
		function initVmrc() {
			   ...
			   $.ajax({
     				url: "vmw_getticket.do?vmName=" + vmName,
      				async: false,
      				cache: false,
   			...
   			if (startupId) {
      			...
      			alert(ipAddress);
     			var connected = vmrc.connect("10.110.28.58", ticket, "", "", vmId, "", "");
     			if (connected) {
...
<body> 
   ...
   <div id="spinner"></div>
   <form id="downloadForm" action="<%=request.getContextPath()%>/vmw_downloadVmrc.do" >
   </form>
</body>
