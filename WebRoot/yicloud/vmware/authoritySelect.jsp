...
<script type="text/javascript">
	...
	var connectid = '<%=request.getAttribute("connectid")%>'; 
	var uuid = '<%=request.getAttribute("uuid")%>';
	var type = '<%=request.getAttribute("type")%>';
	var vtype = '<%=request.getAttribute("vtype")%>';
	...
	function saveOper(){   
		if (userId!='null') {
			if(confirm("您确定要修改对 " + name +" 的权限吗？")){
				$.ajax({
			         type: "get",
			         url: "vmauth_updateUserOperAuthority.do?userid="+ userId+'&connectid='+connectid+'&uuid=' +uuid +'&type='+type+'&vtype='+vtype+'&name='+name+'&'+$("#theForm").serialize(),
			         dataType: "json",
			         success : function(data){
						window.parent.refresh(userId,connectid,uuid,type,vtype,name);
			        }
				});
...
<body >
<s:form action="vmauth_saveUserVirAuthority" method="post" id="theForm">
		<div class="table-ct">
