...
<script type="text/javascript">
	...
	function listExp(){
		...
		var hostip = theForm.queryVHIP.value;
		var type = theForm.queryType.value;
		var state = theForm.queryState.value; 
		var eq_id= $('#EQ_ID').val();
		var url="<%=excel_export_jsp%>?key=vmhostinfo&eq_id="+eq_id+"&name=" + name + "&hostip=" + hostip;
		if (type > 0) {
		    url = url + "&type=" + type;
		}
		if (state != "") {
	...
	function showDatastoreList(vm_uuid){
		$.dialog({
			id:'showStoreList',
			title:'存储块列表',
			resize:false,
			...
			height: '600px',
			lock:true,
			content: 'url:VmDatastoreRelation_datastoreListForVm.do?vmDatastoreRelationObj.vm_uuid='+vm_uuid
		});
		}
...
<body class="mainbody">
<s:form action="showvm_listvm.do" method="post" cssClass="theForm" id="theForm">
<s:hidden id="EQ_ID" name="theForm.EQ_ID"></s:hidden>
<s:hidden id="flag" name="theForm.flag"></s:hidden>
<div class="pd-20 bgcolor-1">
	<div class="box on">
        <h2 class="utt-1">虚拟机列表</h2>
        	<div class="bord-1 pd-10">
               <div class="clearfix filtrate-area">
        		<div class="filtrate-field">
        			<label class="fl">名称:</label>
					<s:textfield name="theForm.queryName" id="queryName" cssClass="inpt-1 fl"></s:textfield>
				</div>
				<div class="filtrate-field">
                	<label class="fl">IP:</label>
					<s:textfield name="theForm.queryVHIP" id="queryVHIP" cssClass="inpt-1 fl"></s:textfield>
				</div>
				<div class="filtrate-field">
					<label class="fl">虚拟化类型:</label>
					<s:select id="queryType" name="theForm.queryType" cssClass="select-1 fl" list="#{'0':'--请选择--','1':'VMWARE','3':'XEN','9':'其他'}" onchange="changeHostIp();"></s:select>
				</div>
				<div class="filtrate-field">
                	<label class="fl">所属主机:</label>
                  	<s:textfield cssClass="inpt-1 fl" name="theForm.queryHostIp" id="hostip"></s:textfield>
					<span class="ubtn-1 mgl-10"><input type="button"  value="选择" name="queryHostIp"/></span>
				</div>
				<div class="filtrate-field">
				...
				<div class="filtrate-field">
					...
					<s:select id="queryState" name="theForm.queryState" cssClass="select-1 fl" list="#{'':'--请选择--','0':'已关闭','1':'正在运行','2':'挂起'}"></s:select>
				</div>
				<div class="filtrate-field">
			...
			<table id = "theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
			  ...
			  <s:iterator value="theForm.resultList" id="theBean">
			  	<tr id="<s:property value="#theBean.VH_UUID"/>" vhid='<s:property value="#theBean.VH_UUID"/>' connID='<s:property value="#theBean.connectId"/>'>
			  		...
			  		<td>
			  			<s:if test="#theBean.name=='' || #theBean.name == null">
		  				  未分配
		  				</s:if>
		  				<s:else>
		  				  已分配
			  		...
			  		<td ><a href="javascript:showDatastoreList('<s:property value="#theBean.VH_UUID"/>')">查看</a></td>
