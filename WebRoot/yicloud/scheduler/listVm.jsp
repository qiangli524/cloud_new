...
<script type="text/javascript">
	...
	function selectVM(){
    	...
    	var vm_id = $('[name=obj.id]:checkbox:checked').attr("title");
    	var vm_name = $('[name=obj.id]:checkbox:checked').attr("vmName");
    	if(flag == 0){
    		api.get("add").listVMInfo(vm_id,vm_name,vmCode,connectId);
    	}else if(flag == 1){
    		api.get("mod").listVMInfo(vm_id,vm_name,vmCode,connectId);
    	}
...
<s:form action="strategy_queryVmHost.do" method="post" id="vmHostObj">
        ...
        <div class="query-form">
                <table width="100%" class="querytable" border="0">
                  <tr>
                    ...
                    <td>
						<s:textfield name="vmHostObj.VH_NAME" id="name"></s:textfield>
                    </td>
                    <td class="til">虚拟化类型:</td>
                    <td>
						<s:select id="type" name="vmHostObj.VH_TYPE" list="#{'':'--请选择--','1':'Vmware','3':'Xen','9':'其他'}" ></s:select>
                    </td>
                  </tr>
                  <tr>
			...
			<table id = "theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
			  ...
			  <s:iterator value="resultList" id="theBean">
			  	<tr id="<s:property value="#theBean.VH_UUID"/>" vhid='<s:property value="#theBean.VH_UUID"/>'>
			  		<td><input type="checkbox" vmCode="<s:property value="#theBean.VH_UUID"/>" 
			  			connectId="<s:property value="#theBean.connectId"/>" title="<s:property value="#theBean.VH_ID"/>" 
			  			vmName="<s:property value='#theBean.VH_NAME'/>" name="obj.id"/></td>
			  		<td> 
