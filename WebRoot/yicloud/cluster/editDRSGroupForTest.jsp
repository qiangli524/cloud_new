...
<head>
 ...
 function submitRequest(thisForm){
       ...
       for (k = 0; k < len; k++)
	   {
	    ...
	    url='cluster_saveEditDRSGroup.do?str='+str+"&name="+encodeURI(encodeURI(name))+"&type="+type+"&cluster_code="+cluster_code;
	   /// document.getElementById('theForm').submit();
	   bar(cluster_code,"正在编辑DRS组");
	   $.getJSON(url,{"time":new Date().toString()},function(data){
				if(data.responseCode == 1){
					barEnd(cluster_code,"成功编辑DRS组");
					w.list();
				}else {
					barEnd(cluster_code,"编辑DRS组失败");
				}
			});
	}
		...
		<input type="hidden" id="grpForm.USERIDS" name="USERIDS" value=""/>
			<table width="100%" border="0" cellspacing="0" class="pop-table nosize">
				<tr >
					...
					<td >
					<s:textfield name="theForm.name" cssStyle="width:220px;   height:20px;" id="name" disabled="true"></s:textfield>
					<span id="name_validate" style="color: RED"></span>
					</td>
