jQuery.fn.rater	= function(options) {
	...
	item.css('width' , 0);
	item.css('z-index' , settings.max / settings.step + 999);
	if (settings.value) {
		item.css('width' , ((settings.value-settings.min)/settings.step+999)*settings.step*settings.width);
	}
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		current_number	= jQuery(this).prevAll('.rater-star-item').size()+1;
