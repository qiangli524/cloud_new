...
<script type="text/javascript">
	function change() {
				document.getElementById('ipconfig').style.display = "";
			} 
	function submitRerult(){
		form.submit();
		return false;
	}
	function deleteManager(ip_id){
		form.action = 'condevice_deleteVirtual.do?ip_id='+ip_id; 
		form.submit();
	}
</script>
<body class="pop-body scrollbody">
		<s:hidden name="theForm.eq_id" id="eq_id" />
			...
			<tr>
				...
				<s:form action="condevice_addVirtualObj.do" id="form" method="post" cssClass="theForm">
				...
				<tr>
					...
					<input type="button" class="thickbox btn-style02" value="确定"
						onclick="javascript:submitRerult(document.getElementById('form'));return false;" />
						</td>
				...
				<s:iterator value="theForm.ipList" id="theBean">
				<tr>
					<td  >
					<s:property value="#theBean.VIRTUAL_IP" />
					</td>
					<td >
					<s:property value="#theBean.DEPART" />
					</td>
					<td colspan="2">
					<a href="#" id="ip_id" onclick="return deleteManager('<s:property value='#theBean.ip_id'/>');">删除此行</a>
					</td>
				</tr>
				</s:iterator >
				<tr id="ipconfig" style="display: none;">
					<td >
					<s:textfield name="theForm.VIRTUAL_IP" id="VIRTUAL_IP" />
						
					</td>
					<td >
					<s:textfield name="theForm.DEPART" id="DEPART" />
