...
<link href="<%=request.getContextPath()%>/ah/paas/statistics/css/statistics.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript">
    	...
    	function showCharts(){
    		...
    		if(kpi_type == "efficiency"){
    			$("#efficiency").insertFusionCharts({
                    swfUrl: "FusionCharts/ZoomLine.swf", 
                    dataSource: "weblogicTab_listServerCharts.do?kpi_type="+kpi_type+"&entity_id="+entity_id+"&cycle_time="+time_efficiency,
                    dataFormat: "jsonurl", 
                    width: "700", 
                    height: "346", 
                    id: "chart1"
               }); 
    		}else if(kpi_type == "response"){
    			$("#response").insertFusionCharts({
                    swfUrl: "FusionCharts/ZoomLine.swf", 
                    dataSource: "weblogicTab_listServerCharts.do?kpi_type="+kpi_type+"&entity_id="+entity_id+"&cycle_time="+time_response,
                    dataFormat: "jsonurl", 
                    width: "700", 
                    height: "346",  
                    id: "chart2"
    			...
    			$("#resource").insertFusionCharts({
                    swfUrl: "FusionCharts/ZoomLine.swf", 
                    dataSource: "weblogicTab_listServerCharts.do?kpi_type="+kpi_type+"&entity_id="+entity_id+"&cycle_time="+time_resource,
                    dataFormat: "jsonurl", 
                    width: "700", 
                    height: "346", 
                    id: "chart3"
    			...
    			$("#capacity").insertFusionCharts({
                    swfUrl: "FusionCharts/ZoomLine.swf", 
                    dataSource: "weblogicTab_listServerCharts.do?kpi_type="+kpi_type+"&entity_id="+entity_id+"&cycle_time="+time_capacity,
                    dataFormat: "jsonurl", 
                    width: "700", 
                    height: "346", 
                    id: "chart4"
    			...
    			$("#exception").insertFusionCharts({
                    swfUrl: "FusionCharts/ZoomLine.swf", 
                    dataSource: "weblogicTab_listServerCharts.do?kpi_type="+kpi_type+"&entity_id="+entity_id+"&cycle_time="+time_exception,
                    dataFormat: "jsonurl", 
                    width: "700", 
                    height: "346", 
                    id: "chart5"
    	...
    	function selectShowType(type){
    		...
    		if(type=='efficiency'){
    			 $("#efficiency").updateFusionCharts({
	   	          	 dataSource: "weblogicTab_listServerCharts.do?kpi="+show_efficiency+"&entity_id="+entity_id+"&kpi_type=efficiency&start_time="+start_time_efficiency+"&end_time="+end_time_efficiency+"&cycle_time="+time_efficiency
	   	         });
    		}else if(type=='response'){
	   			 $("#response").updateFusionCharts({
	   	          	 dataSource: "weblogicTab_listServerCharts.do?kpi="+show_response+"&entity_id="+entity_id+"&kpi_type=response&start_time="+start_time_response+"&end_time="+end_time_response+"&cycle_time="+time_response
	   	         });
			}else if(type=='resource'){
	   			 $("#resource").updateFusionCharts({
	   	          	 dataSource: "weblogicTab_listServerCharts.do?kpi="+show_resource+"&entity_id="+entity_id+"&kpi_type=resource&start_time="+start_time_resource+"&end_time="+end_time_resource+"&cycle_time="+time_resource
	   	         });
			}else if(type=='capacity'){
	   			 $("#capacity").updateFusionCharts({
	   	          	 dataSource: "weblogicTab_listServerCharts.do?kpi="+show_capacity+"&entity_id="+entity_id+"&kpi_type=capacity&start_time="+start_time_capacity+"&end_time="+end_time_capacity+"&cycle_time="+time_capacity
	   	         });
			}else if(type=='exception'){
	   			 $("#exception").updateFusionCharts({
	   	          	 dataSource: "weblogicTab_listServerCharts.do?kpi="+show_exception+"&entity_id="+entity_id+"&kpi_type=exception&start_time="+start_time_exception+"&end_time="+end_time_exception+"&cycle_time="+time_exception
	   	         });
			}
    	}
  ...
  <s:form action="" id="theForm">
  	<div class="scrollbody">
						...
						<div class="top">
							<div class="ml5"><font color="orange" size="3" style="margin-right: 10%;">负载</font>
								<select id="show_efficiency" onchange="selectShowType('efficiency')" style="width:120px;">
									<option value="PS-05-04-004">GC调用数</option>
									<option value="PS-05-04-001">页面请求处理数</option>
									<option value="PS-05-02-001">线程池线程总数</option>
									<option value="PS-05-04-002">线程池暂挂请求数</option>
									<option value="PS-05-07-002">工作管理器请求数</option>
								</select>
								<span>
							  		时间：
			...
			<s:elseif test="kpi_type=='response'">
				<div class="mainbox-kpi" style="width: 100%;border: 0px solid #FFFFFF;">
		 		<div class="item">
					<div class="top">
						<div class="ml5"><font color="orange" size="3" style="margin-right: 10%;">响应</font>
							<select id="show_response" onchange="selectShowType('response')" style="width:120px;">
								<option value="PS-05-05-003">GC调用时间</option>
								<option value="PS-05-05-001">页面请求处理时间</option>
							</select>
							<span>
						  		时间：
					...
					<div class="top">
						<div class="ml5"><font color="orange" size="3" style="margin-right: 10%;">资源</font>
							<select id="show_resource" onchange="selectShowType('resource')" style="width:120px;">
								<option value="PS-05-07-007">CPU占用率</option>
								<option value="PS-05-04-005">GC时间成本</option>
								<option value="PS-05-07-001">活动套接字数</option>
								<option value="PS-05-07-003">JVM堆使用率</option>
								<option value="PS-05-07-004">JVM线程峰值数</option>
								<option value="PS-05-07-006">JVM堆内存使用量</option>
								<option value="PS-05-07-005">JVM非堆内存使用量</option>
							</select>
							<span>
						  		时间：
					...
					<div class="top">
						<div class="ml5"><font color="orange" size="3" style="margin-right: 10%;">容量</font>
							<select id="show_capacity" onchange="selectShowType('capacity')" style="width:120px;">
								<option value="PS-05-02-002">线程池空闲线程数</option>
							</select>
							<span>
						  		时间：
					...
					<div class="top">
						<div class="ml5"><font color="orange" size="3" style="margin-right: 10%;">异常</font>
							<select id="show_exception" onchange="selectShowType('exception')" style="width:120px;">
								<option value="PS-05-06-004">JVM死锁线程数</option>
								<option value="PS-05-06-002">工作管理器粘滞线程数</option>
							</select>
							<span>
						  		时间：
