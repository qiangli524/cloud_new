...
$(function(){
	...
	$("input[name=checkboxId]").change(function(){
			...
			$.ajax({
				type:'post',
				url:'oracleTab_queryBusiNameAndLimitListByInstanceEntityId.do?entity_id='+entity_id,
				dataType:'json',
				success:function(data){
					$("#busiCount").text(data.busiList.length);
					$("#tbTab tr:not(:first)").remove(); 
					var resultList = data.busiList;
					if (resultList != null) {
						for ( var i = 0; i < resultList.length; i++) {
							$("<tr><td style=\"padding-left: 20%;\"><a href=\"#\" onclick='showBusiness(\""+resultList[i].id +"\")'><span class=\"value\">"
											+ resultList[i].entity_name
											+ "</span></a></td><td style=\"padding-left: 20%;\"><span class=\"percentage\"><b style=\"width: "+resultList[i].limit +"\"></b></span>"
											+ resultList[i].limit
											+ "</td></tr>")
									.insertAfter(
											$("#tbTab tr:eq(" + i+ ")"));
			...
			$.ajax({
				type:'get',
				dataType:'text',
				url:'oracleTab_showInstanceChartByEntityId.do?entity_id='+entity_id+'&name='+entity_name+"&cycle_time="+time,
	...
	//展示业务摘要
	$("[name='showBusiness']").click(function(){
     	$td=$(this).parent();
        var id=$td.attr("busi_id");
        $.dialog({
       		id:'showBusiness',
       		title:'信息',
       		height:'550px',
       		width:'900px',
       		lock:true,
       		content:'url:paasTree_tabs.do?node_type=22&server_type=3'+'&id='+id
       	});
	});
});
	...
	function showAverage(){
		...
		$.ajax({
			type:'get',
			dataType:'text',
			url:'oracleTab_showLimitAverage.do?id='+id+"&cycle_time="+time,
			success:function(msg){
	...
	function queryChart(){
		...
		$.ajax({
			type:'get',
			dataType:'text',
			url:'oracleTab_showLimitAverage.do?id='+id+"&start_time="+start_time+"&end_time="+end_time+"&cycle_time="+time,
			success:function(msg){
...
<body class="pop-body scrollbody">
						...
						<tbody>
							<s:iterator value="resultList" id="theBean">
								<tr>
									...
									<td align="center">
										<s:if test="#theBean.entity_type == 32">
											Oracle Instance
										</s:if>
										<s:else>
											—
									...
									<td align="center">
										<s:property value="#theBean.busiCount"/>
									</td>
							  		<td align="center">
	...
	<div class="mainboxservice" style="overflow-y: auto;width: 100%;">
		<div class="item" style="border: 0px solid #FFFFFF;">
			<div class="top">
				<div class="ml5"><font color="orange" size="3">业务列表</font>
				<font color="orange" size="3" style="margin-left: 82%;" id="busiCount"><s:property value="paasEntityObj.busiCount"/></font><font color="orange" size="3">个</font>					
				</div>
			</div>
			<div>
				<div class="right" style="width: 100%;margin-bottom: 5px;margin-top: 10px;">
					<table width="100%" class="ued-table" border="0" cellspacing="0" id="tbTab" >
						...
						<tbody>
							<s:iterator value="businessList" id="theBean">
								<tr>
									<td style="padding-left: 20%;" busi_id="<s:property value='#theBean.id'/>">
										<a href="#" name="showBusiness"><span class="value"><s:property value="#theBean.entity_name"/></span></a>
									</td>
							  		<td style="padding-left: 20%;">
										<span class="percentage">
												<b style='width: <s:property value="#theBean.limit" default="0"/>'>
												</b>
										</span>&nbsp;
										<s:property value="#theBean.limit" default="0"/>
									</td>
								</tr>
							</s:iterator>
						</tbody>
