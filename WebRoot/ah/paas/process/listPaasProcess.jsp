	...
	$(function(){
        ...
        $("[name='addprocess']").click(function(){
	  		$.dialog({
    			...
    			height: '380px',
    		    lock:true,
    			content: 'url:treeprocess_addPaasProcess.do?oper=add&example_id='+example_id+'&hostIP='+hostIP+'&type='+type
	    		});
        });
        ...
        $("[name='editProcess']").unbind().live("click",function(){
            ...
            var processId=$td.siblings("[processid]").attr("processid");
	  		$.dialog({
    			...
    			height: '380px',
    		    lock:true,
    			content: 'url:treeprocess_addPaasProcess.do?oper=edit&processid='+processId
	    		});
        });
	 ...
	           });
      ...
      function saveProcess(theForm,selectUsers,userips,oper,example_id2,type1){
    	  mask('正在编辑进程,请稍后....','0.5','0px');
       	 $.ajax({
            type: "GET",
            url: "treeprocess_savePaasProcess.do?"+theForm+"&selectUsers="+selectUsers+"&userips="+userips+"&oper="+oper+"&example_id="+example_id2+"&type1="+type1,
            dataType: "json",
            success : function(data){
	               removeMask();
	               $("#theForm").submit();
...
<s:form action="treeprocess_listPaasProcess.do" method="post" cssStyle="theForm" id="theForm">
