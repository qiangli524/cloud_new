...
(function ($) {
	...
	$.addFlex = function (t, p) {
				...
				$('thead tr:first th:visible', this.hDiv).each(function () {
					...
					if (isNaN(cdpos)) {
						...
						$('div:eq(' + n + ')', g.cDrag).css({
							'left': (!(browser.mozilla) ? cdpos - cdcounter : cdpos) +22 + 'px'
						}).show(); 
					}else{  
						$('div:eq(' + n + ')', g.cDrag).css({
		...
		$(t).before(g.bDiv);
		$(g.bDiv).css({height: (p.height=='auto') ? 'auto' : p.height+ (p.height.toString().indexOf('%')>0 ? "":"px")
		}).scroll(function (e) {
			g.scroll()
		}).append(t);
		if (p.height == 'auto') {
...
(function($){
	$.fn.UED_datagrid = function(option){
	...
	$(function(){
		$('table[data-role="UED_datagrid"]').UED_datagrid();
	})
 })(jQuery)
