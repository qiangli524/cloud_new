...
var errMsg = {
    ...
    num_letter: {
        ...
        test: function(obj) {
            ...
            var patrn = "1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ`~!@#$%^&*()-_=+|\{}[]:;'?/,.<>";
    ...
    mac: {
        ...
        test: function(obj) {
            ...
            var sInput = obj.value;
            var type = $(obj).attr("macType");
            if (type=="bar" && sInput.search(reg1) == -1) {//以“-”作为分隔符
                return false;
            }else if(type =="colon" && sInput.search(reg2) == -1){//以“：”作为分隔符
            	return false;
            }
            return true;
        }
	...
	$(document).ready(function () {
	    ...
	    for (var j = 0;j < forms.length;j++){
		    for (var i = 0; i < forms[j].elements.length; i++) {
		       ...
		       $(forms[j].elements[i]).bind(" keyup blur",function(){validateField(this);});
