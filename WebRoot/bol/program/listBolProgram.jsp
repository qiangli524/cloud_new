...
<script type="text/javascript">
	...
	function addRequest(){
		$.dialog({
			...
			height: '225px',
			lock:true,
			content: 'url:bolprogram_addRequest.do?oper=add'
		});
	}
	...
	function updateRequest(){
		...
		$.dialog({
			...
			height: '225px',
			lock:true,
			content: 'url:bolprogram_updateRequest.do?oper=update&programId='+programId
		});
	}

	function saveBolProgram(programVO,oper){
		if(oper=='add'){
		...
		theForm.action = "bolprogram_saveBolProgram.do?name="+ programVO.name + "&status=" + programVO.status +"&descrip="+ programVO.descrip + "&id=" + programVO.id + "&oper=" + oper;
		theForm.submit();
	}

	function deleteRequest(){
		if(confirm("确认要删除应用程序？")){
			var programId = '';
			var couterNum = 0;
			var checkboxids = document.getElementsByName("checkboxid");
			if(checkboxids!=null&&checkboxids.length>0){
			...
			var theForm = document.getElementById("theForm");
			theForm.action = "bolprogram_delBolProgram.do?programId="+ programId;
			theForm.submit();
		}
	}
...
<s:form action="bolprogram_listBolProgram.do" id="theForm" method="post" cssClass="theForm">
	...
	<table width="100%" class="blue-table sorttable" border="0" id="theTable"
		...
		<thead>
			<tr>
				<th>选择</th>
				<th onclick="sort(theTable,1,'string')">应用标识</th>
				...
				<th onclick="sort(theTable,5,'string')">应用描述</th>
			</tr>
		</thead>
		<tbody>
