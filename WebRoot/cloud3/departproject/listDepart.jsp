...
<head>
	...
	<script type="text/javascript">
    	...
    	$(function(){
            $("#addForm").click(function(){
	    		...
	    		w.$.dialog({
	    		    ...
	    		    min: true,
	    		    lock:true,
	    			content: 'url:depart_editDepart.do?obj.parent_id='+parent_id
	    		});
              });
        	...
        	currentEdit=$(this);
        	var depart_id = currentEdit.attr("depart_id");
    		w.$.dialog({
    			id:'vdi',
    			title:'详细信息',
    			width: '1000px',
              ...
              $("[name='use']").click(function(){
	    		...
	    		w.$.dialog({
	    		    ...
	    		    min: true,
	    		    lock:true,
	    			content: 'url:depart_departResourceRate.do?departId='+id
	    			});
	              });
		...
		function deleteTem(){
			...
			id = $('[name=obj.id]:checkbox:checked').val();
			var url="depart_deleteDepart.do?obj.id="+id;
			if(confirm("确认要删除吗")){
	...
	function expertExcel(){
			...
			var url = "depart_exportDepartExcel.do?obj.parent_id="+$("#parent_id").attr("value");
			obj.action = url;
			obj.submit();
		}        
...
<body class="pop-body scrollbody" onLoad="self.focus();document.obj.name.focus()">
    <s:form action="depart_listDepart.do" method="GET" id="obj" >
    <s:hidden name="obj.parent_id" id="parent_id"></s:hidden>
    
    <h2 class="utt-1">科室管理</h2>
...
<!-- 				value="查询" onclick="javascript:list();" /> -->
       ...
       <div class="table-ct">
            <table width="100%" border="0" cellspacing="0" class="blue-table sorttable" id="theTable">
            	<thead>
					<tr>
						<th>选择</th>
						<th onclick="sort(theTable,1,'string')">科室名称</th>
						<th onclick="sort(theTable,2,'string')">科室负责人</th>
