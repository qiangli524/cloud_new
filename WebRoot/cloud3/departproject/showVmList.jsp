...
<script type="text/javascript">
	...
	function queryVM(){
		...
		vmHostObj.action='departproject_queryVMByObj.do?vmHostObj.PROJECT_ID='+project_id;
		vmHostObj.submit();
	}
	//重置
	function resetVM(){
	...
	function exportExcel(){
		var project_id = '<s:property value="vmHostObj.PROJECT_ID" />';
		document.forms[0].action ="departproject_exportVmExcel.do?vmHostObj.PROJECT_ID=" + project_id;
                  ...
                  <tr>
                    <td colspan="8" class="btns">
                        <div>
							...
							<input type = "button" class="btn-style02" value = "导出" onclick = "javascript:exportExcel()" />
                        </div>
                    </td>
                  </tr>
                </table>
			...
			<table id="theTable" width="100%" class="blue-table sorttable" border="0" cellspacing="0">
			  ...
			  <tbody>
			   <s:iterator value="selectedList" id="selectedList">
			  	<tr>
			  		<td ><s:property value="#selectedList.VH_NAME"/></td>
			  		<td><s:property value="#selectedList.VH_IP"/></td>
			  		<td><s:if test="#selectedList.VH_TYPE == 3">
			  			XEN
			  		</s:if><s:else>
			  			Vmware
			  		</s:else>
			  		</td>
			  		<td><s:property value="#selectedList.VH_SYSTEM"/></td>
			  		<td><s:property value="#selectedList.VH_CPU"/>核</td>
			  		<td><s:property value="#selectedList.VH_MEM"/>MB</td>
			  		<td><s:property value="#selectedList.VH_STORAGE"/>MB</td>
			  		<!-- <td id="stateText">
			  			<s:if test="#selectedList.VH_STAT==1">
							正在运行
						</s:if>
						<s:if test="#selectedList.VH_STAT==0">
							已关闭
						</s:if>
						<s:if test="#selectedList.VH_STAT==2">
							挂起
						</s:if>
			  		</td> -->
			  	</tr>
