...
<body>
	<div class="mainbody">
		<s:form action="resourcestatistics_viewVM.do" method="post"
			cssClass="theForm" id="theForm">
			<s:hidden id="vtype" name="vtype"></s:hidden>
			<s:hidden id="status" name="status"></s:hidden>
			<s:hidden id="clusterList" name="clusterList"></s:hidden>
					...
					<table id="theTable" width="100%" class="blue-table sorttable"
						...
						<thead>
							<tr>
								...
								<th onclick="sort(theTable,2,'string')">电源状态</th>
						...
						<tbody>
							<s:iterator value="vmList" id="theBean">
								<tr>
									...
									<td><s:if test="#theBean.VH_TYPE==2">
			  				XEN虚拟化
			  				...
			  				VMWARE虚拟化
								...
								<%--	<td><s:if test='#theBean.connectStatus=="connected"'>正常链接</s:if>
										...
										<s:elseif test='#theBean.connectStatus=="orphaned"'>孤立的</s:elseif>
										<s:elseif test='#theBean.connectStatus=="inaccessible"'>不可访问的</s:elseif>
										<s:else>不可用</s:else></td>
								</tr> --%>
									<td><s:property value="@java.lang.Math@round(#theBean.VH_STORAGE/1024)" />GB</td>
 							</s:iterator>
						</tbody>
					</table>
