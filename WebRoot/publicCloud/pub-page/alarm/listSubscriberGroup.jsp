    ...
    <script type="text/javascript">
        $(function () {
            ...
            $("#add").unbind().live('click', function () {
                var url = "url:subscriberGroup_createSubscriberGroup.do";
                $.dialog({
                    id: 'addSubscriberGroup',
                    title: '添加告警组',
                    //height: 60,
                    height: 510,
                    width: 600,
            ...
            $("#edit").unbind().live('click', function () {
                if ($('input[name="bb"]:checked').length == 1) {
                    ...
                    var url = "url:subscriberGroup_updateSubscriberGroupPage.do?obj.id=" + id;
                    $.dialog({
                        id: 'editSubscriberGroup',
                        title: '修改告警组',
                        height: 510,
                        width: 600,
                        lock: true,
            ...
            //删除
            function deleteSubscriberGroup() {
                mask("正在删除告警组", "info");
                var id = '';
                $('input[name="bb"]:checked').each(function () {
                    id = $(this).attr("objId");
                ...
                if ($('input[name="bb"]:checked').length == 1) {
                    $.ajax({
                        url: "subscriberGroup_deleteSubscriberGroup.do?obj.id=" + id,
                        type: "POST",
                        data: "text",
                        dataType: "json",
                        success: function (data) {
                            ...
                            if (data.result == "success") {
                                mask("删除告警组成功！", "success", 3000);
                                $("#theForm").submit();
                            } else {
                                mask("删除告警组失败！", "error", 3000);
                            }
                        }
                    });
                } else {
        ...
        function saveSubscriberGroup(url, params) {
            mask("正在保存告警组", "info");
            $.ajax({
                url: url,
                type: "POST",
                ...
                success: function (data) {
                    removeMask();
                    if (data.result == "success") {
                        mask("保存告警组成功！", "success", 3000);
                        $("#theForm").submit();
                    } else {
                        mask("保存告警组失败！", "error", 3000);
                    }
                }
            });
        }
        function updateSubscriberGroup(url, params) {
            mask("正在修改告警组", "info");
            $.ajax({
                url: url,
                type: "POST",
                ...
                success: function (data) {
                    removeMask();
                    if (data.result == "success") {
                        mask("修改告警组成功！", "success", 3000);
                        $("#theForm").submit();
                    } else {
                        mask("修改告警组失败！", "error", 3000);
                    }
                }
            });
        }
...
<div class="container">
    ...
    <div class="col-c7">
        ...
        <div class="main-c1 fr">
            ...
            <!--标题 end-->
            <!--文字介绍 star-->
            <p class="p-con4">告警组管理 <font color="red"> </font> <a href="javascript:history.back();">返回</a></p>
            <!--文字介绍 end-->
            <!--pd-15 star-->
            <div class="pd-15">
                <div class="col-c8">
                    <form id="theForm" action="subscriberGroup_list.do?type=<s:property value="type"/>" method="post">
                        <ul class="operate-list">
                            ...
                            <li class="search">
                                <input type="text" class="text-1" placeholder="名称" name="obj.name" id="name"
