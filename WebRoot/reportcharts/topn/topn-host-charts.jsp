...
<html>
    <head>
        ...
        $(document).ready(function(){
           ...
           /*$("#chartContainer").bind(fusioncharts + "dataloaded", function (e, args) {
           });*/
         ...
         function getChartsEntity(type,topname) {
    	   ...
    	   if (topname != undefined) {
      	   ...
      	   if (type == 'cpu') {
      		//插入报表数据源
          	  $("#chartContainer").insertFusionCharts({
	                 ...
	                 dataSource: "entityTopN_queryHostTopN.do?" + params, 
	                 dataFormat: "jsonurl", 
	                 width: "99%", 
	                 height: "400px", 
	                 id: "topnHostCharts"
      	   ...
      	   var url = "entityTopN_queryHostTopNList.do?" + params;
      	   $.getJSON(url,{'time':new Date().toString()},function(data){
      			if (data != null) {
	                 for(var i = 0; i < data.length ;i++){
	                	 ...
	                	 if (typeof(data[i].store_usage)=="undefined"){
	                		 data[i].store_usage=0;
	                	 }
	                  $("<tr><td>"+data[i].name+"</td><td>"
	                   			 +data[i].ip+"</td><td>"
	                   			 +data[i].cpu_usage+"%</td><td>"
	                   			 +data[i].mem_usage+"%</td><td>"
	                   			 +data[i].store_usage+"%</td></tr>").insertAfter($("#tab tr:eq("+i+")"));
	                 }
      			}
		   });
         }
